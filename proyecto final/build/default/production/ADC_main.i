...
{ ADCON0bits.CHS=0;
    ...
    ADCON0bits.ADON=1;
    _delay((unsigned long)((3)*(20000000/4000000.0)));
}
void main(void) {
    uint8_t string_tmp[10];
    float string_volt[10];
    ...
    while(1){
      ...
      ValorADC_H = ADRESH<<2;
      ValorADC_L = ADRESL>>6;
      ValorleidoADC = ValorADC_H + ValorADC_L;
      volt=ValorleidoADC;
      XLCDgotoXY(0,0);
      putrsXLCD("VALORES: ");
      XLCDgotoXY(0,11);
      volt=((volt*14)/1023);
      sprintf(string_volt,"%0.2f",volt);
      XLCDgotoXY(1,0);
      putrsXLCD("Ph: ");
      putsXLCD(string_volt);
      putrsXLCD(" ");
      _delay((unsigned long)((200)*(20000000/4000.0)));

               if( 6.5>=volt || volt<7.5 ){
             if(PORTBbits.RB4==0){
